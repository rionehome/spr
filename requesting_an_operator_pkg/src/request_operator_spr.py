			...
			for body in person.body_part:
				...
				if 14 <= body.part_id < 18:
					if min_point[0] > body_image_x:
						min_point = (body_image_x, body_image_y, body.z)
					if max_point[0] < body_image_x:
						max_point = (body_image_x, body_image_y, body.z)
				if body.part_id == 0:
					center = (body_image_x, body_image_y, body.z)
			# 鼻が認識していない場合
			if center[1] == -1:
				center = (
					(min_point[0] + max_point[0]) / 2, min_point[1] + max_point[1] / 2, min_point[2] + max_point[2] / 2)
			if center[2] > 2.5:
				continue
			x = int(min_point[0])
